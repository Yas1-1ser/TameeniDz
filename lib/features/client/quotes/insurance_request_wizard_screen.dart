...
// ─────────────────────────────────────────────────────────────────────────────
// ٢ - طلب التأمين  (Insurance / Policy Application Wizard)
// Flow: اختر الخدمة → معلومات شخصية + NIN → رفع الوثائق → تأكيد
// الفرق عن التسعيرة: الحالة الأولية 'insurance_pending'، لون العنوان أزرق-ذهبي،
//                    رسالة النجاح مختلفة، وحقل إضافي لمبلغ التأمين المطلوب.
// ─────────────────────────────────────────────────────────────────────────────

class InsuranceRequestWizardScreen extends ConsumerStatefulWidget {
  const InsuranceRequestWizardScreen({super.key, this.extra});
  final Object? extra;

  @override
  ConsumerState<InsuranceRequestWizardScreen> createState() =>
      _InsuranceRequestWizardScreenState();
}

class _InsuranceRequestWizardScreenState
    extends ConsumerState<InsuranceRequestWizardScreen> {
  ...
  // ── Colours (gold-teal theme to differentiate from quote) ───────────────
  static const _accentColor = Color(0xFF00695C);   // teal-green
  static const _gold        = AppColors.goldAccent;
  ...
  Future<Map<String, String>> _uploadDocuments(String userId) async {
    ...
    for (var i = 0; i < _requiredDocs.length; i++) {
      ...
      final path =
          'quotes/$userId/insurance_${spec.key}_${DateTime.now().millisecondsSinceEpoch}.$ext';
  ...
  Future<void> _submit(AppLocalizations l10n) async {
    ...
    try {
      ...
      final premium = 0.0; // Operator sets the actual price

      await ref.read(policyRepositoryProvider).createPolicy({
        'client_id':           user.id,
        'plan_id':             _plan!.id,
        'operator_id':         _plan!.operatorId,
        'status':              'insurance_pending',  // ← نوع مختلف
        'amount':              premium,
        ...
        'metadata': {
          ...
          'request_type':      'insurance',         // ← نوع الطلب
  ...
  void _nextStep(AppLocalizations l10n) {
    if (_step == 0) {
      if (_nameCtrl.text.trim().isEmpty ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(l10n.pleaseFillAllFieldsCorrectly)),
        );
        return;
      }
      setState(() => _step = 1);
      ...
      SliverPadding(
        ...
        sliver: SliverList(
          delegate: SliverChildBuilderDelegate((ctx, i) {
            ...
            ).animate().fadeIn(delay: (60 * i).ms).slideX(begin: 0.06, end: 0);
  ...
  Widget _buildHeader(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
            colors: [_accentColor, _accentColor.withValues(alpha: 0.88)]),
        borderRadius:
  ...
  Widget _buildStepForm(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionTitle(Icons.person_outline_rounded, l10n.policyholderInfo),
          const SizedBox(height: 16),
          _tealField(l10n.fullName, l10n.fullNameHint, _nameCtrl),
          const SizedBox(height: 14),
          _tealField(l10n.phoneLabel, l10n.phoneHint, _phoneCtrl,
              keyboard: TextInputType.phone),
          const SizedBox(height: 14),
          _tealField(l10n.ninLabel, l10n.ninHint, _ninCtrl),
          const SizedBox(height: 28),
          _sectionTitle(
              Icons.folder_open_rounded, l10n.requiredDocsForService),
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child: const Icon(Icons.verified_rounded,
              color: _accentColor, size: 56),
        ),
        const SizedBox(height: 16),
        Text(
          l10n.insuranceRequestSuccess,
          textAlign: TextAlign.center,
          style: GoogleFonts.amiri(
        ...
        Container(
          ...
          child: Text(
            l10n.insuranceSuccessDesc,
            textAlign: TextAlign.center,
            style: GoogleFonts.ibmPlexSansArabic(
                fontSize: 14, height: 1.6, color: const Color(0xFF2D1F0E)),
          ),
        ),
  ...
  Widget _buildBottomCta(
    ...
    final label = _step == 0
        ? l10n.nextUploadDocs
        : (_submitting ? l10n.uploading : l10n.sendInsuranceRequest);
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: enabled ? _gold : colors.slate200,
            foregroundColor:
                enabled ? const Color(0xFF2D1F0E) : colors.slate500,
            shape: RoundedRectangleBorder(
  ...
  Widget _tealField(String label, String hint, TextEditingController ctrl,
  ...
  Widget _docPreviewCard(QuoteDocumentSpec spec, bool done, String lang) =>
      Container(
        ...
        child: Row(children: [
          Icon(spec.icon, color: _accentColor),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(18),
          border: Border.all(
            color: hasFile ? _accentColor : _gold.withValues(alpha: 0.45),
            width: hasFile ? 2 : 1.5,
          ),
        ),
        child: Row(children: [
          Icon(
            ...
            color: hasFile ? _accentColor : _gold,
          ),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
