...
class AtPoliciesScreen extends ConsumerStatefulWidget {
  const AtPoliciesScreen({super.key});

  @override
  ConsumerState<AtPoliciesScreen> createState() => _AtPoliciesScreenState();
}

class _AtPoliciesScreenState extends ConsumerState<AtPoliciesScreen> {
  String _searchQuery = '';
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: PortalLayout(
        selectedIndex: 2,
        portalTitle: l10n.operatorTakaful,
  ...
  Widget _buildList(List<PolicyModel> policies, AppLocalizations l10n) {
    ...
    var filtered = policies.where((p) {
      final nin = (p.nin ?? '').toLowerCase();
      return p.applicantName.toLowerCase().contains(q) ||
  ...
  Widget _policyCard(PolicyModel policy, AppLocalizations l10n) {
    return GestureDetector(
      onTap: () => context.push('/at/application/${policy.id}'),
      child: Container(
