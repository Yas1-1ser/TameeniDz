...
class AtClaimsScreen extends ConsumerStatefulWidget {
  const AtClaimsScreen({super.key});

  @override
  ConsumerState<AtClaimsScreen> createState() => _AtClaimsScreenState();
}

class _AtClaimsScreenState extends ConsumerState<AtClaimsScreen>
  ...
  PolicyStatus _mapStatus(String? status) {
    ...
    if (status == 'under_review') return PolicyStatus.pending; // Show in pending tab until fully processed
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: PortalLayout(
        selectedIndex: 3,
        portalTitle: l10n.algeriaTakafulPortal,
  ...
  void _showDocumentViewer(BuildContext context, String label, String url) {
    final l10n = AppLocalizations.of(context)!;
    showDialog(
      context: context,
      builder: (ctx) => Dialog(
            ...
            children: [
              ...
              if (url.endsWith('.pdf'))
                ...
                ClipRRect(
                  ...
                  child: Image.network(url, fit: BoxFit.contain, height: 300, errorBuilder: (c, e, s) => const Icon(Icons.broken_image, size: 80)),
                ),
              const SizedBox(height: 16),
              TextButton.icon(
                onPressed: () => Navigator.pop(ctx),
                icon: const Icon(Icons.close),
                label: Text(l10n.closeDialog, style: GoogleFonts.ibmPlexSansArabic()),
              ),
            ],
          ),
        ),
  ...
  Widget _buildLineChartCard(List<Map<String, dynamic>> items, AppLocalizations l10n) {
    ...
    if (spots.length < 2) {
      ...
      spots.add(const FlSpot(1, 120));
      spots.add(const FlSpot(2, 85));
      spots.add(const FlSpot(3, 200));
    }
  ...
  Widget _buildClaimsList(List<Map<String, dynamic>> items, AppLocalizations l10n) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return InkWell(
          onTap: () => context.push('/at/claim/$id?source=$source'),
          borderRadius: BorderRadius.circular(kRadiusMd),
          child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Row(
                    children: [
                      Text(
                        item['client_name'] ?? l10n.takafulClient,
                        style: GoogleFonts.ibmPlexSansArabic(
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                          color: kInk,
              ...
              Builder(builder: (context) {
                ...
                final documents = item['documents']; // from wizard
                ...
                if (documents is List && documents.isNotEmpty) {
                  for (var doc in documents) {
                    if (doc is Map) {
                      ...
                      if (docUrl != null) {
                        docButtons.add(_buildDocButton(context, title, docUrl, Icons.file_present_rounded));
                      }
                    }
                  }
                  ...
                  if (nationalIdUrl != null) {
                    docButtons.add(_buildDocButton(context, l10n.nationalIdCard, nationalIdUrl, Icons.badge_rounded));
                  }
                  if (proofUrl != null) {
                    docButtons.add(_buildDocButton(context, l10n.proofOfResidence, proofUrl, Icons.home_rounded));
                  }
                }
                ...
                return Column(
                  ...
                  children: [
                    const SizedBox(height: 12),
                    Text(
                      l10n.clientDocuments,
                      style: GoogleFonts.ibmPlexSansArabic(fontSize: 13, fontWeight: FontWeight.bold, color: kInkMuted),
                    ),
